defmodule M.Shop.AggregateEmitter do
  @doc """
  To generate Shop.Aggregate-s.

  All generated aggregates are collected in M.Shop.Registry.
  """
  use GenServer
  ...
  def init(_args) do
    ...
    aggregate = M.Repo.Shop

    M.Shop.pubsub_repo_query()
  ...
  def handle_info({:object, aggregate, id: id}, %{aggregate: aggregate} = state) do
    ...
    case Registry.lookup(@registry, {:object, aggregate, id: id}) do
      [] ->
        ...
        {:ok, pid} = M.Shop.Aggregate.start_link(id: id)
